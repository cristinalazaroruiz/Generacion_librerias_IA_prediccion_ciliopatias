...
#Guardamos como anexo 2
write.csv(df_no_supervised, file = "Anexo_2.csv")
...
grid.arrange(clust_ward_euclidean, clust_ward_manhattan, 
             clust_single_euclidean, clust_single_manhattan, nrow =2)
...
fviz_cluster(kmeans.result, data = df_no_supervised_scaled, xlab = "",
             ylab ="")+ggtitle("Kmeans con 3 centroides", subtitle = "")+
...
ggplot(df_cluster2, aes(x = type, fill = Cluster))+geom_bar()+
  labs(title = "Distribucion pacientes en clusters kmeans con distancia euclidiana",
...
ggplot(df_cluster_manhattan2, aes(x = type, fill = cluster))+geom_bar()+
  labs(title = "Distribucion pacientes en clusters kmeans con distancia Manhattan",
       x = "Tipos de ciliopatía", y = "Pacientes")
